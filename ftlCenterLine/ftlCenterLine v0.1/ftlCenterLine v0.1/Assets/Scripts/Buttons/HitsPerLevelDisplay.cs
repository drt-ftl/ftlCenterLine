...
public class HitsPerLevelDisplay : ftlCenterLineManager
{	
	void OnEnable()
	{
	...
	void OnGUI()
	{
		...
		if (NumberOfHitsPerLevel == 21)
			GUI.Label(GuiRect, "Hits/Level: Infinity", keyTextStyle);
		else
			GUI.Label(GuiRect, "Hits/Level: " + NumberOfHitsPerLevel.ToString(), keyTextStyle);
	}
}
