...
public class CheckButtons
{
	...
	public void CheckAllButtonsHit(GameObject hit)
	{
		...
		switch (hit.name) 
		{
			...
			{
				hit.GetComponent<GeneralButton>().press ();
			if (ftlCenterLineManager.StartingLevel < 4)
				ftlCenterLineManager.StartingLevel++;
				break;
			}
			case "StartingLevelDown":
			{
				hit.GetComponent<GeneralButton>().press ();
				if (ftlCenterLineManager.StartingLevel > 1)
				ftlCenterLineManager.StartingLevel--;
				break;
			}
			case "HitsUp":
			{
				hit.GetComponent<GeneralButton>().press ();
			if (ftlCenterLineManager.NumberOfHitsPerLevel < 21)
				ftlCenterLineManager.NumberOfHitsPerLevel++;
				break;
			}
			case "HitsDown":
			{
				hit.GetComponent<GeneralButton>().press ();
			if (ftlCenterLineManager.NumberOfHitsPerLevel > 3)
				ftlCenterLineManager.NumberOfHitsPerLevel--;
				break;
			}
			default:
